...
#define AEOPENGL_LIBSYNC_STUB_H

#include <errno.h>
#include <poll.h>

static inline int sync_wait(int fd, int timeout_ms)
{
    struct pollfd pfd;
    int ret;
...
LDFLAGS="-L${termux_prefix}/lib -Wl,-rpath,${MESA_TERMUX_RUNPATH} -llog -Wl,--no-as-needed -landroid-shmem -Wl,--as-needed" \
